...
**技术栈：** Python · ChromaDB · TF-IDF · 中文分词 · FastAPI

构建了企业级 RAG 检索引擎，支持混合检索（关键词 + 向量）、持久化热加载、生产级API。

- 基于 TF-IDF 向量空间模型实现中文语义检索，无需外部模型下载
- 双路召回：倒排索引关键词匹配（35%权重）+ TF-IDF向量相似度（65%权重）
- 零依赖中文分词：双字滑窗 + 英文单词提取，纯正则实现
- ChromaDB持久化存储 + JSON缓存索引，服务重启秒级热加载（<100ms）
- 完整 FastAPI 接口：/search、/refresh、/stats、/health，支持跨平台调用
- 生产级检索性能：592个知识块、18,755个关键词项，检索延迟<50ms

**成果：** 7个服务的生产级RAG后端，支持混合检索和语义理解，零外部API依赖。
